    ...
    @if (Cart::instance('cart')->count() > 0 &&
                            ...
                            <div class="ps-product--cart-mobile">
                                <div class="ps-product__thumbnail">
                                    <a href="{{ $product->original_product->url }}"><img
                                            src="{{ $cartItem->options['image'] }}" alt="{{ $product->name }}" /></a>
                                </div>
                                <div class="ps-product__content">
                                    <a class="ps-product__remove remove-cart-item" href="#"
                                    ...
                                    <p class="mb-0">
                                        <small>
                                            <span class="d-inline-block">{{ $cartItem->qty }} x</span> <span
                                                class="cart-price">{{ format_price($cartItem->price) }} @if ($product->front_sale_price != $product->price)
                                    ...
                                    <p class="mb-0">
                                        <small><small>{{ $cartItem->options['attributes'] ?? '' }}</small></small></p>
        ...
        <div class="ps-cart__footer">
            @if (EcommerceHelper::isTaxEnabled())
                <h5>{{ __('Sub Total') }}:<strong>{{ format_price(Cart::instance('cart')->rawSubTotal()) }}</strong>
                </h5>
                {{-- <h5>{{ __('Tax') }}:<strong>{{ format_price(Cart::instance('cart')->rawTax()) }}</strong></h5> --}}
                <h3>{{ __('Total') }}:<strong>{{ format_price(Cart::instance('cart')->rawSubTotal() + Cart::instance('cart')->rawTax()) }}</strong>
                </h3>
            @else
                <h3>{{ __('Sub Total') }}:<strong>{{ format_price(Cart::instance('cart')->rawSubTotal()) }}</strong>
                </h3>
            @endif
            <figure>
